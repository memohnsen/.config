...
(after! doom-modeline
  ...
  (defun mm/modeline-mode-face ()
    ...
    (pcase (and (bound-and-true-p evil-local-mode) evil-state)
      ...
      ('vterm 'mm-modeline-mode-terminal)
  ...
  (defun mm/modeline-git-branch ()
    ...
    (when-let ((root (mm/modeline-git-root)))
  ...
  (defun mm/modeline-git-diff ()
    "Return lualine-style file diff counts."
    (when (and buffer-file-name (mm/modeline-git-root))
      (let* ((default-directory (mm/modeline-git-root))
             (file (file-relative-name buffer-file-name default-directory))
             (stat (ignore-errors
                     (car (process-lines "git" "diff" "--numstat" "--" file)))))
        (when (and stat (not (string-empty-p stat)))
          (pcase-let ((`(,added ,deleted . ,_) (split-string stat "\t")))
            (let ((parts nil))
              (unless (string= added "0")
                (push (propertize (concat "+" added) 'face 'doom-modeline-info) parts))
              (unless (string= deleted "0")
                (push (propertize (concat "-" deleted) 'face 'doom-modeline-urgent) parts))
              (when parts
